...
export function createCredentialsApi(baseUrl: string) {
    ...
    const userPath = (realmId: string, username: string) =>
        `${basePath(realmId)}/${encodeURIComponent(username)}`;

    return {
        list: (realmId: string): Promise<UserPass[]> => apiGet<UserPass[]>(baseUrl, basePath(realmId)),

        get: (realmId: string, username: string): Promise<UserPass> =>
            apiGet<UserPass>(baseUrl, userPath(realmId, username)),

        create: (realmId: string, userpass: UserPass): Promise<UserPass> =>
            apiPost<UserPass>(baseUrl, basePath(realmId), userpass),
        ...
        delete: (realmId: string, username: string): Promise<void> =>
            apiDelete(baseUrl, userPath(realmId, username)),
    };
}
