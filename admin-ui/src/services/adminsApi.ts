...
export function createAdminsApi(baseUrl: string) {
    ...
    const adminRealmPath = (adminId: string, realmId: string) =>
        `${adminPath(adminId)}/realms/${encodeURIComponent(realmId)}`;
    ...
    return {
        ...
        update: (adminId: string, admin: Admin): Promise<Admin> =>
            apiPut<Admin>(baseUrl, adminPath(adminId), admin),
