...
export function createRealmsApi(baseUrl: string) {
    return {
        ...
        get: (realmId: string): Promise<Realm> => apiGet<Realm>(baseUrl, `${REALMS_PATH}/${encodeURIComponent(realmId)}`),
        ...
        delete: (realmId: string): Promise<void> => apiDelete(baseUrl, `${REALMS_PATH}/${encodeURIComponent(realmId)}`),
    };
}
