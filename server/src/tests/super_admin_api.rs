...
/// Authenticate as `admin` in the `_` realm and return the ready-to-use client.
/// The `_ea_` session cookie is stored inside the returned [`AuthClient`] and will
/// be sent automatically on all subsequent requests.
async fn authenticate_as_admin(
    ...
    assert!(
        cookie.is_some(),
        "Expected a session cookie to be issued after authentication"
    );
    info!(
        "Authenticated as admin; session cookie name: {:?}",
        cookie.unwrap().name()
    );
    Ok(client)
}
...
/// the given `realm_id` in their `realms` list, then return an authenticated client.
///
...
async fn create_and_authenticate_realm_admin(
    ...
    // Register userpass credentials in ADMIN_REALM
