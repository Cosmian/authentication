...
impl Database for SqliteDatabase {
    ...
    async fn get_oauth_client(&self, client_id: &str) -> AuthDbResult<Option<OAuthClient>> {
        ...
        Ok(row.map(|r| OAuthClient {
            ...
            redirect_uris: json_to_vec(
                &r.try_get::<String, _>("redirect_uris").unwrap_or_default(),
            ),
            grant_types: json_to_vec(&r.try_get::<String, _>("grant_types").unwrap_or_default()),
            response_types: json_to_vec(
                &r.try_get::<String, _>("response_types").unwrap_or_default(),
            ),
    ...
    async fn list_oauth_clients_by_realm(&self, realm: &str) -> AuthDbResult<Vec<OAuthClient>> {
        ...
        Ok(rows
            ...
            .map(|r| OAuthClient {
                ...
                grant_types: json_to_vec(
                    &r.try_get::<String, _>("grant_types").unwrap_or_default(),
                ),
                response_types: json_to_vec(
                    &r.try_get::<String, _>("response_types").unwrap_or_default(),
                ),
    ...
    async fn consume_authorization_code(
        ...
        // Delete unconditionally so a reused code cannot be consumed twice.
    ...
    async fn get_refresh_token(&self, token_hash: &[u8]) -> AuthDbResult<Option<RefreshToken>> {
        ...
        Ok(row.map(|r| RefreshToken {
            ...
            created_at: r.try_get("created_at").unwrap_or_default(),
            revoked: r.try_get::<i64, _>("revoked").unwrap_or_default() != 0,
        }))
    }
