...
impl Database for MySqlDatabase {
    async fn init(&self) -> AuthDbResult<()> {
        ...
        sqlx::query(
            ...
            CREATE TABLE IF NOT EXISTS vault_secret_ids (
                secret_id_accessor  VARCHAR(255) PRIMARY KEY NOT NULL,
    ...
    async fn get_vault_role(&self, role_name: &str) -> AuthDbResult<Option<VaultRole>> {
        ...
        .fetch_optional(&self.pool)
        .await?;
        row.map(|r| vault_role_from_mysql_row(&r)).transpose()
    }
    ...
    async fn destroy_vault_secret_id_by_accessor(
        ...
        sqlx::query("DELETE FROM vault_secret_ids WHERE role_name = ? AND secret_id_accessor = ?")
            .bind(role_name)
            .bind(accessor)
            .execute(&self.pool)
            .await?;
    ...
    async fn find_vault_token_by_hash(&self, hash: &[u8]) -> AuthDbResult<Option<VaultToken>> {
        ...
        .fetch_optional(&self.pool)
        .await?;
        row.map(|r| vault_token_from_mysql_row(&r)).transpose()
    }
...
fn vault_role_from_mysql_row(r: &sqlx::mysql::MySqlRow) -> AuthDbResult<VaultRole> {
    use sqlx::Row as _;
...
fn vault_token_from_mysql_row(r: &sqlx::mysql::MySqlRow) -> AuthDbResult<VaultToken> {
    use sqlx::Row as _;
