...
impl Database for PostgresDatabase {
    async fn init(&self) -> AuthDbResult<()> {
        ...
        sqlx::query(
            ...
            CREATE TABLE IF NOT EXISTS vault_roles (
                ...
                token_ttl       BIGINT  NOT NULL DEFAULT 3600,
                token_policies  TEXT    NOT NULL DEFAULT '[]',
                secret_id_ttl   BIGINT  NOT NULL DEFAULT 0,
    ...
    async fn get_vault_role(&self, role_name: &str) -> AuthDbResult<Option<VaultRole>> {
        let row = sqlx::query(
            "SELECT role_name, role_id, token_ttl, token_policies, secret_id_ttl, bind_secret_id, created_at FROM vault_roles WHERE role_name = $1",
        )
        .bind(role_name)
        .fetch_optional(&self.pool)
        .await?;
        row.map(|r| vault_role_from_pg_row(&r)).transpose()
    }

    async fn get_vault_role_by_role_id(&self, role_id: &str) -> AuthDbResult<Option<VaultRole>> {
        let row = sqlx::query(
            "SELECT role_name, role_id, token_ttl, token_policies, secret_id_ttl, bind_secret_id, created_at FROM vault_roles WHERE role_id = $1",
        )
        .bind(role_id)
        .fetch_optional(&self.pool)
        .await?;
    ...
    async fn delete_vault_role(&self, role_name: &str) -> AuthDbResult<()> {
        sqlx::query("DELETE FROM vault_roles WHERE role_name = $1")
            .bind(role_name)
            .execute(&self.pool)
            .await?;
        Ok(())
    ...
    async fn find_vault_secret_id_by_hash(
        ...
        let row = sqlx::query(
            "SELECT secret_id_accessor, secret_id_hash, role_name, expiry_time, created_at FROM vault_secret_ids WHERE secret_id_hash = $1",
        )
        .bind(hash)
        .fetch_optional(&self.pool)
        .await?;
    ...
    async fn find_vault_token_by_hash(&self, hash: &[u8]) -> AuthDbResult<Option<VaultToken>> {
        let row = sqlx::query(
            "SELECT token_hash, role_name, policies, ttl, renewable, expiry_time, created_at FROM vault_tokens WHERE token_hash = $1",
        )
        .bind(hash)
        .fetch_optional(&self.pool)
        .await?;
        row.map(|r| vault_token_from_pg_row(&r)).transpose()
    }

    async fn delete_vault_token_by_hash(&self, hash: &[u8]) -> AuthDbResult<()> {
        sqlx::query("DELETE FROM vault_tokens WHERE token_hash = $1")
            .bind(hash)
            .execute(&self.pool)
            .await?;
        Ok(())
    ...
    async fn cleanup_expired_vault_entries(&self) -> AuthDbResult<()> {
        ...
        sqlx::query("DELETE FROM vault_tokens WHERE expiry_time > 0 AND expiry_time < $1")
            .bind(now)
            .execute(&self.pool)
            .await?;
        sqlx::query(
            "DELETE FROM vault_secret_ids WHERE expiry_time IS NOT NULL AND expiry_time < $1",
        )
        .bind(now)
        .execute(&self.pool)
        .await?;
...
fn vault_role_from_pg_row(r: &sqlx::postgres::PgRow) -> AuthDbResult<VaultRole> {
    use sqlx::Row as _;
    ...
    Ok(VaultRole {
        ...
        bind_secret_id: r.try_get("bind_secret_id")?,
        created_at: r.try_get("created_at")?,
    })
}

fn vault_token_from_pg_row(r: &sqlx::postgres::PgRow) -> AuthDbResult<VaultToken> {
    use sqlx::Row as _;
