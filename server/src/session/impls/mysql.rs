...
impl MySqlSessionStore {
    ...
    #[allow(dead_code)]
    /// Create a new MySqlSessionStore from a connection string
    pub async fn from_url(url: &str) -> AuthResult<Self> {
        let pool = MySqlPool::connect(url)
            .await
            .map_err(|e| AuthError::Generic(format!("Failed to connect to MySQL: {e}")))?;
        Ok(Self::new(pool))
    }
    ...
    pub async fn init(&self) -> AuthResult<()> {
        sqlx::query(
            ...
            CREATE TABLE IF NOT EXISTS session (
                ...
                INDEX idx_session_stale_at (stale_at),
                INDEX idx_session_realm_user (realm_id, username, auth_scheme)
            )
            "#,
        )
        .execute(&self.pool)
...
impl SessionStore for MySqlSessionStore {
    ...
    async fn get_sessions_for_clients(
        ...
        let query_str = format!(
            ...
            FROM session
            WHERE realm_id = ? AND stale_at > ? AND (created_at + max_age_seconds) > ?
            AND ({})
            "#,
            user_conditions
        );
