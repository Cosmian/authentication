...
use crate::{AuthenticatedClientScheme, models::AuthScheme};
...
/// - Return the default username from Authentication server parameters if configured
...
impl EnsureAuth {
    /// Creates a new `EnsureAuth` middleware
    ///
    /// # Parameters
    ...
    S::Future: 'static,
{
    ...
    fn call(&self, req: ServiceRequest) -> Self::Future {
        ...
        // Insert the default username as the authenticated user if configured
        if let Some(ref username) = self.default_username {
            debug!(
                "No authentication configured, using default username: {}",
                username
            );
