...
|---------|---------|-------------|
| `_server` | No | Enables `actix-web::ResponseError` for `AuthError` and sqlx error variants. Used only by the `auth_server` crate — do not enable in API server code. |

---

## Testing

The client library has no standalone integration tests (its behaviour is tested through `auth_server`'s test suite). To run unit tests:
